...
def library():
    ...
    registry = archive.read_json(paths.SHOWS_JSON, [])
...
def run_transcription(job):
    ...
    logs = paths.CACHE / 'podcast_asr'
    logs.mkdir(parents=True, exist_ok=True)
    error_log = logs / (job['id'] + '.log')
    command = [str(ASR_PYTHON), '-B', str(paths.APP/'podcast_transcribe.py'),
